---
title: "Resolución de agentes externos"
...
Cada resultado de descubrimiento implementa [`agent-source-v1.schema.json`](https://github.com/BaldrVivaldelli/baldr-router/blob/v0.20.0/contracts/agent-source-v1.schema.json). Cada candidato contiene:
...
El preview implementa [`agent-catalog-sync-v1.schema.json`](https://github.com/BaldrVivaldelli/baldr-router/blob/v0.20.0/contracts/agent-catalog-sync-v1.schema.json) e incluye un digest del baseline del registro y la propiedad de la fuente. Aplicar dos veces el mismo resultado no hace nada. Una definición modificada del agente crea una versión exacta nueva; el sincronizador nunca sobrescribe un `AgentRef` o digest existente.
...
`BALDR_AGENT_REGISTRY_PATH` puede seleccionar otro archivo. El archivo debe implementar [`agent-registry-v1.schema.json`](https://github.com/BaldrVivaldelli/baldr-router/blob/v0.20.0/contracts/agent-registry-v1.schema.json) y está limitado a 1 MiB y 1.000 manifiestos.

Hay un ejemplo compatible con Kiro en [`examples/agents.local.json`](https://github.com/BaldrVivaldelli/baldr-router/blob/v0.20.0/examples/agents.local.json). Apunta al adaptador `kiro-cli` y nombre de agente existentes; el agente Kiro permanece externo.
...
La decisión durable implementa [`agent-team-resolution-v1.schema.json`](https://github.com/BaldrVivaldelli/baldr-router/blob/v0.20.0/contracts/agent-team-resolution-v1.schema.json).
...
El primer conector independiente y la implementación persistente del manager se describen en [`external-agent-http.md`](../external-agent-http/). Agregan el transporte `http-json` de solo lectura, resolución remota exacta, contratos de catálogo y salud, y el selector de agentes de VS Code.

El SDK, runner local, frontera del workspace y ciclo de vida de ejecución se describen en [`external-agent-runtime.md`](../external-agent-runtime/).
